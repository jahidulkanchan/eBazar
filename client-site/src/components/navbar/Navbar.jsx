    ...
    >
      <div className="container mx-auto py-2 md:py-1.5 px-2 md:px-5 flex gap-5 justify-between items-center">
        ...
        <div className="relative -right-2 -top-5 block md:hidden">
            <div onClick={()=>setIsHidden(!isHidden)} className="cancel-btn cursor-pointer text-2xl w-fit absolute right-0 top-0"><RxCross2 /></div>
            </div>
        ...
        <Link to='/signin' className="px-3 bg-white rounded flex items-center gap-1.5 border-amber-400 py-1 border">
          <FaUser className="text-amber-400" /> <span>Sign In</span>
        </Link>
        </div>
        <div className="flex md:hidden items-center gap-4">
          <div className="relative md:hidden">
            <Link to='/signin'><span className="group text-2xl text-amber-500"><FaRegCircleUser /></span></Link>
          </div>
        {/* Hambarger icon */}
        <div onClick={()=>setIsHidden(!isHidden)} className={`${isHidden? '' : 'text-amber-500'} bar-icon cursor-pointer text-xl`}>
        <FaBarsStaggered />
        </div>
        </div>
      </div>
      <div className="min-h-[50px] container mx-auto px-1.5 md:px-5 flex justify-between items-center bg-amber-400">
        ...
        <div className="shopping-info flex items-center gap-2 md:gap-3">
          <Link to='/' className="text-2xl mt-1"><IoMdHeartEmpty /></Link>
          <Link to='/' className="text-2xl"><LiaShoppingBagSolid /></Link>
        </div>
      </div>
    </nav>
   </>
